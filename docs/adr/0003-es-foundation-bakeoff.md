...
**Done** — the Commanded foundation is promoted into the real tree
(`feat/commanded-foundation`):

- `Latchkey.EventStore`, `Latchkey.CommandedApp`, `Latchkey.Router` — infra, wired
  into the supervision tree (disabled in `:test`; integration tests start it).
- `Latchkey.PropertyManagement.Tenancy` — framework-free fold + decide (the domain
  home, grown by hand); `…Tenancy.Aggregate` is the thin Commanded shell over it.
- `Latchkey.PropertyManagement.Arrears` (Ash read model) + `…ArrearsProjector`.
- Aggregate unit tests (no DB) + a full-stack integration test (event store + projector).

The throwaway spike (`spike/es-bakeoff`, PR #19) remains the evidence record; the
pure-Ash spike stays documented there as the fallback if the seam proves thin.

## Next

Grow the PM domain on this core — the rest of L1–L8 and the exit/overstay accrual —
then the Accounts context + **ACL-1** seam, where the seam-weight bet gets tested
for real. Revisit the decision if that seam turns out thinner than assumed.
